...
def shuaguai(img_1,img_2,img_3,img_da):   #img_old,img_new):
...
if __name__ == "__main__":
    ...
    l1 = matchimg('F:\\apk\BILAN\chuji.png','F:\\apk\BILAN\datu.png')   #  第一个参数是本地的小图片，第二个参数是截取到的游戏窗口图，函数为在第二个图片上寻找第一个图片的坐标
    #print (l1)
    ...
    img = ImageGrab.grab(bbox=(initxl, inityl, initxr, inityr))  # 根据坐标截取屏幕
    # img.show()
    img.save('F:\\apk\BILAN\jintuda1.png')
    time.sleep(1)
    l2 = matchimg('F:\\apk\BILAN\jintu1.png', 'F:\\apk\BILAN\jintuda1.png')
    #print(l2)
    ...
    img = ImageGrab.grab(bbox=(initxl, inityl, initxr, inityr))  # 根据坐标截取屏幕
    # img.show()
    img.save('F:\\apk\BILAN\jintuda2.png')
    time.sleep(1)
    l3 = matchimg('F:\\apk\BILAN\jintu2.png', 'F:\\apk\BILAN\jintuda2.png')
    ...
    img = ImageGrab.grab(bbox=(initxl, inityl, initxr, inityr))  # 根据坐标截取屏幕
    # img.show()
    img.save('F:\\apk\BILAN\jintuda3.png')
    time.sleep(1)
    l4 = matchimg('F:\\apk\BILAN\jintu3.png', 'F:\\apk\BILAN\jintuda3.png')
    ...
    img = ImageGrab.grab(bbox=(initxl, inityl, initxr, inityr))  # 根据坐标截取屏幕
    # img.show()
    img.save('F:\\apk\BILAN\daguaida1.png')
    time.sleep(2)
    l5 = matchimg('F:\\apk\BILAN\daguai1.png', 'F:\\apk\BILAN\daguaida1.png')

    # print(l2)
    l55 = list(l5)  # 函数的返回值是一个元组，转换为列表
    l555 = zuobiao(l55)
    LeftClick(initxl + random_in_range(l555[0], l555[2]), inityl + random_in_range(l555[1], l555[3]))  # 点击立刻前往
